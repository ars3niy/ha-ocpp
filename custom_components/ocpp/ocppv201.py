...
class ChargePoint(cp):
    ...
    async def set_charge_rate(
        ...
        if profile:
            raise HomeAssistantError(
                ...
                translation_key="profile_not_supported",
            )
        if (limit_amps < 32) and (limit_watts < 22000):
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="dual_profile_not_supported",
            )
    ...
    def on_authorize(self, id_token: dict, **kwargs):
        ...
        if (token_type == IdTokenType.iso14443) or (token_type == IdTokenType.iso14443):
